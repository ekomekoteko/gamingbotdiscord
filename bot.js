...
const secreT = [
 ...
 client.on('message', message => {
   if (message.content.startsWith("g!خواطر ")) {
                if(!message.channel.guild) return message.reply('** This command only for servers**');
...
client.on('guildCreate', guild => {
  client.channels.get("499206701904297984").send(`:white_check_mark: **تم اضافة البوت في سيرفر جديد مبروكك
...
client.on('guildDelete', guild => {
  client.channels.get("499206701904297984").send(`:negative_squared_cross_mark: **MA4y yla fe dhya 
Server name: __${guild.name}__
Server owner: __${guild.owner}__
Server id: __${guild.id}__ 
Server Count: __${guild.memberCount}__**`)
...
client.on('guildMemberAdd', member => {
    ...
    client.channels.get('501864104856780800').setName(`⟫『 ${memberCount} عدد الاعضاء 』⟪`);
